...
resources. XSEDE machines are far larger (by at least an order of magnitude) than what most universities offer. But transitioning from a campus resource to an XSEDE resource is seldom a trivial task but XSEDE has
  taken many steps to make this easier, including the Campus Bridging initiative, the Campus Champions
program, the Extended Collaborative Support Service (ECSS) program, and through education and outreach.

  In this paper, our team of biologists and application support analysts (includ-
ing a Campus Champion) dissect a computationally intensive biology project and share the insights we have gained
  ...
  recombination rates of tens of genome profiles using the mlRho, a serial,
  open-source, genome analysis code. For the initial investigation, we initially estimated that we would need
...
\abhi{talk about how the faster mlrho is awesome. for xsede+other users
why everyone should attempt analysis}
...
step, because we found that this was faster. \abhi{Needs fixing, see Bernhard's comment: 
 - Figure 3: The drop in performance between 2.0 and 2.1 can only be due
 to my switch between single step reading/writing and iterated
 reading/writing. On my machine this sped up the program, but apparently
 not on yours. This contradicts the phrase "because we found that this
 was faster". Now, there was also the issue of slowdown due to concurrent
 analyses of large files. It appeared to me from Abhinav's feedback in
 January that switching from single-step i/o to iterated i/o mitigated
 this problem. If so, then "because we found that this was faster" should
 be amended accordingly. If not, we should switch back to 2.0 (see my
 change log at the end of mlRhoDoc.pdf)
}
