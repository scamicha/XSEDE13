...
\maketitle
\begin{abstract}
  An increasing number of researchers' computational demands have outgrown the capacity of desktop
  workstations and are turning to supercomputers to run their simulations and calculations. But many of
  today's computational problems require larger resource commitments than what individual universities can
  provide. XSEDE is one of the first places researchers turn to when they outgrow their campus
  resources. XSEDE machines are far larger (at least by an order of magnitude) than what most universities
  offer. Transitioning from a campus resource to a far larger XSEDE resource is seldom a trivial task. XSEDE has
  taken many steps to make this easier, including the Campus Bridging initiative, the Campus Champions
  program, Extended Collaborative Support Service (ECSS) program, and through education and outreach.

  In this paper, we dissect a computationally intensive biology project and share the insights we have gained
  to help strengthen the programs mentioned above. Our team consists of biologists and application support
  analysts (including a Campus Champion). We worked on a project to calculate population mutation and
  recombination rates of tens of genome profiles using the mlRho code. The mlRho software is a serial,
  open-source, genome analysis code. For the initial investigation proposed we estimated that we would need
  ...
  {\bf CAMPUS BRIDGIST} to play in bridging the gaps between existing programs and present mlRho as an ideal
  example case.
\end{abstract}
...
of species. Thus far we have conducted ML recombination rate studies on a total of 46 individual
...
a reasonable period of time. Quarry is a 2960 core machine, where as XSEDE machines such as Ranger, Stampede,
...
one node at a time. Given that IU resource, Quarry, had a serial queue, which placed multiple serial jobs from
...
the schedulers are configured to prioritize larger jobs. Many XSEDE centers provide workarounds for this
...
based on the size of their genome: a small genome {\it F. cylindrus}, a medium genome {\it
  P. ornithorhynchus}, and a large genome {\it C. familiaris}. Data sizes are provided in table
...
concurrent instances. The results are shown in figure~\ref{fig:bj-scaling}. Figure~\ref{fig:bj-scaling} shows
that the distance traveled, irrespective of the genome, size and the number of concurrent instances, increases
...
Ranger with the improved version of mlRho as it was in the process decommissioning.
...
help. In the case of the mlRho project, research staff from PTI helped biologists to prepare an allocation
...
Ranger and Kraken are appropriate and we have not experienced extremely long wait times with past BigJob
...
process of implementation on Kraken.
