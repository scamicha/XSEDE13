...
resources. XSEDE machines are far larger (by at least an order of magnitude) than what most universities offer. Transitioning from a campus resource to an XSEDE resource is seldom a trivial task but XSEDE has
...
program, the Extended Collaborative Support Service (ECSS) \cite{ecss_web} program, and through education and outreach.
  ...
  campus bridging expert to play in bridging the gaps between existing programs and present mlRho as an ideal case study.
\end{abstract}
...
\keywords{high-throughput, XSEDE, BigJob, pilot-job, genetics, mlRho}
...
across thousands of processor cores, depending on the genome sizes of species. Thus far we have
conducted maximum likelihood (ML) recombination rate studies on a total of 46 individual genomes. In addition
to the use of the BigJob tool, we have done an extensive performance analysis of the mlRho code and, with biologists
and computer scientists working in conjunction, have been able to improve the runtime of the code by a
...
a single diploid individual. Such data are collected, for example, for the 1000 human genome project. mlRho reads a profile consisting of the number of nucleotides \texttt{A}s, \texttt{C}s, \texttt{G}s, and \texttt{T}s from a file at each sequenced position.
...
analyze all of our target genomes without sacrificing any of the objectives. At this point, we were using
...
central difference is in the size of the machines and therefore the biggest challenge was scaling up our workflow
accordingly. 
%\begin{table}
%\centering
...
Max Planck Institute for Evolutionary Biology (MPI) in Germany. Following initial benchmarking and scalability
...
step, because we found that this helped with stability on the Lustre filesystem.
